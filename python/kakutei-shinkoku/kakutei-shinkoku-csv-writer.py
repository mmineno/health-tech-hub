    ...
    if os.path.exists(OUTPUT_CSV):
        try:
            ...
            df = pd.read_csv(
                ...
                on_bad_lines='warn'     # エラー行は警告を表示してスキップ
            )
            ...
            log_error(f"CSVの読み込みエラー: {OUTPUT_CSV}\n{e}")
            print("CSV読み込みエラーのため処理を中断します。")
        ...
        for filename in sorted(files):
            ...
            try:
                ...
                for idx, pil_img in enumerate(pil_images, start=1):
                    # PDFの場合でも元のファイル名を使用する（_page1を付けない）
                    file_id_for_csv = file_path
